    ...
    <div class="relative overflow-hidden bg-slate-950 rounded-3xl shadow-2xl p-6 md:p-10 text-white">
        ...
        <div class="relative z-10 flex flex-col lg:flex-row justify-between items-center gap-8">
            <div class="space-y-4 text-center lg:text-left">
                <div class="inline-flex items-center gap-2 px-3 py-1.5 bg-orange-500/10 border border-orange-500/20 rounded-full">
                    <span class="relative flex h-2 w-2">
                        <span class="animate-ping absolute inline-flex h-full w-full rounded-full bg-orange-400 opacity-75"></span>
                        <span class="relative inline-flex rounded-full h-2 w-2 bg-orange-500"></span>
                    </span>
                    <span class="text-[10px] font-bold uppercase tracking-[0.15em] text-orange-200">Periode Aktif: {{ $tahunSekarang }}</span>
                </div>
                <div>
                    <h1 class="text-3xl md:text-5xl font-extrabold tracking-tight mb-2">
                        Dashboard <span class="text-transparent bg-clip-text bg-gradient-to-r from-orange-400 to-amber-200">SIRAMBO</span>
                    </h1>
                    <p class="text-slate-400 text-sm md:text-base font-medium max-w-xl leading-relaxed">
                        Sistem Monitoring & Pelaporan Data PDRB Wilayah <span class="text-white border-b border-orange-500/50">{{ $namaWilayah }}</span>.
                    </p>
                </div>
            </div>
            ...
            <div class="flex items-center gap-6 glass-effect p-5 md:p-6 rounded-2xl shadow-2xl min-w-[240px]">
                <div class="text-right">
                    <p class="text-[10px] font-bold uppercase tracking-widest text-orange-400 mb-1">{{ \Carbon\Carbon::now()->translatedFormat('l, d F Y') }}</p>
                    <p id="clock" class="text-3xl md:text-4xl font-black tracking-tighter tabular-nums"></p>
                </div>
                <div class="h-12 w-[1px] bg-white/10"></div>
                <div class="bg-gradient-to-br from-orange-500 to-orange-600 p-3 rounded-xl shadow-lg shadow-orange-500/30">
...
<div class="bg-blue-50 border-l-4 border-blue-500 rounded-xl p-4">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div class="stat-card bg-white p-6 rounded-2xl border border-slate-200/60 flex items-center justify-between">
            <div>
                <span class="text-[9px] font-bold text-slate-400 uppercase tracking-widest block mb-1">{{ $s['label'] }}</span>
            ...
            <div class="w-12 h-12 rounded-xl flex items-center justify-center {{ $s['color'] == 'orange' ? 'bg-orange-50 text-orange-500' : 'bg-blue-50 text-blue-600' }}">
        ...
        <form method="GET" class="flex flex-col md:flex-row items-end gap-4">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 w-full">
                <div class="space-y-1.5">
                    ...
                    <select name="mode" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm font-semibold text-slate-700 focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500 outline-none transition-all" onchange="this.form.submit()">
                        <option value="tahunan" {{ $currentMode == 'tahunan' ? 'selected' : '' }}>📊 Tahunan</option>
                        <option value="triwulanan" {{ $currentMode == 'triwulanan' ? 'selected' : '' }}>📈 Triwulanan</option>
                    </select>
                </div>
                <div class="space-y-1.5">
                    <label class="text-[10px] font-extrabold text-slate-500 uppercase ml-1">Periode Tahun</label>
                    <select name="id_tahun" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm font-semibold text-slate-700 focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500 outline-none transition-all" onchange="this.form.submit()">
                        @foreach($tahun as $t)
                            <option value="{{ $t->id_tahun }}" {{ $idTahun == $t->id_tahun ? 'selected' : '' }}>🗓️ Tahun {{ $t->tahun }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="space-y-1.5">
                    <label class="text-[10px] font-extrabold text-slate-500 uppercase ml-1">Triwulan</label>
                    <select name="id_periode" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm font-semibold text-slate-700 focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500 outline-none transition-all" onchange="this.form.submit()">
                        @foreach($periode as $p)
                            <option value="{{ $p->id_periode }}" {{ $idPeriode == $p->id_periode ? 'selected' : '' }}>📈 {{ $p->nama_periode }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="bg-white p-6 rounded-2xl border border-slate-200/60 shadow-sm">
            ...
            <div class="grid grid-cols-2 gap-4 text-sm">
                ...
                <div>
                    ...
                    <p class="text-xl font-black {{ abs($diskrepansiBerlaku) <= 1 ? 'text-emerald-600' : 'text-red-600' }}">
        ...
        <div class="bg-white p-6 rounded-2xl border border-slate-200/60 shadow-sm">
            ...
            <div class="grid grid-cols-2 gap-4 text-sm">
                ...
                <div>
                    <p class="text-slate-400 font-semibold">Diskrepansi</p>
                    <p class="text-xl font-black {{ abs($diskrepansiKonstan) <= 1 ? 'text-emerald-600' : 'text-red-600' }}">
    ...
    <div class="bg-white p-6 md:p-8 rounded-[2.5rem] border border-slate-200/60 shadow-sm">
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
            <div>
                <h2 class="text-sm font-black text-slate-900 uppercase tracking-widest flex items-center gap-2">
                    <span class="w-2 h-2 bg-orange-500 rounded-full animate-pulse"></span> Monitoring Kelengkapan Data PDRB
                </h2>
        ...
        <div class="grid grid-cols-1 xl:grid-cols-2 gap-10">
            <div class="space-y-4">
                <h3 class="text-[15px] font-black uppercase text-orange-600 border-b border-orange-100 pb-2">Lapangan Usaha</h3>
                <div class="h-[400px]">
                    <canvas id="monitoringChart"></canvas>
                </div>
            </div>
            <div class="space-y-4">
                <h3 class="text-[15px] font-black uppercase text-blue-600 border-b border-blue-100 pb-2">Pengeluaran</h3>
                <div class="h-[400px]">
    ...
    <div class="bg-white p-6 md:p-8 rounded-[2.5rem] border border-slate-200/60 shadow-sm">
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
            <div>
                <h2 class="text-sm font-black text-slate-900 uppercase tracking-widest flex items-center gap-2">
                    <span class="w-2 h-2 bg-purple-500 rounded-full animate-pulse"></span> Monitoring Kelengkapan Fenomena
                </h2>
                <p class="text-xs text-slate-400 mt-1 font-medium">Status entri data fenomena (nilai & rating) per wilayah</p>
            </div>
        ...
        <div class="grid grid-cols-1 xl:grid-cols-2 gap-10">
            ...
            <div class="space-y-4">
                <h3 class="text-[15px] font-black uppercase text-purple-600 border-b border-purple-100 pb-2">Kelengkapan Data Fenomena (Lapangan Usaha)</h3>
                <div class="h-[400px]">
            ...
            {{-- Tabel Persentase Kelengkapan --}}
<div class="space-y-4">
    <h3 class="text-[15px] font-black uppercase text-purple-600 border-b border-purple-100 pb-2">Persentase Kelengkapan per Wilayah</h3>
    <div class="overflow-x-auto max-h-[400px] custom-scrollbar">
        <table class="w-full text-sm">
            <thead class="sticky top-0 bg-white">
                <tr class="border-b border-slate-200">
                    <th class="text-left py-3 px-2 text-[10px] font-black text-slate-500 uppercase">Wilayah</th>
                    <th class="text-center py-3 px-2 text-[10px] font-black text-slate-500 uppercase">Terisi</th>
                    <th class="text-center py-3 px-2 text-[10px] font-black text-slate-500 uppercase">Total</th>
                    <th class="text-center py-3 px-2 text-[10px] font-black text-slate-500 uppercase">Persentase</th>
                    <th class="text-center py-3 px-2 text-[10px] font-black text-slate-500 uppercase">Status</th>
                  </tr>
            </thead>
            <tbody>
                @foreach($fenomenaLabels as $index => $label)
                    ...
                    <tr class="border-b border-slate-100 hover:bg-slate-50 transition">
                        ...
                        <td class="text-center py-3 px-2 font-mono font-bold text-slate-800">{{ number_format($terisi) }}</td>
                        <td class="text-center py-3 px-2 font-mono font-bold text-slate-800">{{ number_format($total) }}</td>
                        <td class="text-center py-3 px-2">
                            <div class="flex items-center justify-center gap-2">
                                <div class="w-16 bg-slate-100 rounded-full h-1.5">
                                    <div class="bg-purple-500 h-1.5 rounded-full" style="width: {{ $persen }}%"></div>
                                </div>
                                <span class="text-xs font-bold text-purple-600">{{ $persen }}%</span>
                            </div>
                        </td>
                        <td class="text-center py-3 px-2">
                            <span class="text-[9px] font-bold px-2 py-1 rounded-full {{ $statusColor }}">{{ $status }}</span>
                        </td>
                    </tr>
                @endforeach
            </tbody>
    ...
    <p class="text-xs text-slate-500">{{ $jumlahWilayah }} wilayah × {{ number_format($totalPerWilayah) }} data</p>
    <p class="text-xs text-purple-500 mt-1">Mode: {{ ucfirst($currentMode) }}
        ...
        @endif
            ...
            <div class="bg-gradient-to-br from-slate-100 to-white rounded-xl p-4 text-center">
                ...
                <p class="text-2xl font-black {{ $targetTercapai ? 'text-emerald-600' : 'text-amber-600' }} mt-1">{{ $persenKeseluruhan }}%</p>
    ...
    <div class="bg-white p-6 md:p-8 rounded-[2.5rem] border border-slate-200/60 shadow-sm">
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
            <div>
                <h2 class="text-sm font-black text-slate-900 uppercase tracking-widest flex items-center gap-2">
                    <span class="w-2 h-2 bg-cyan-500 rounded-full animate-pulse"></span> Monitoring Fenomena - Pengeluaran
                </h2>
                <p class="text-xs text-slate-400 mt-1 font-medium">Status entri data fenomena pengeluaran per wilayah</p>
            </div>
        </div>

        <div class="grid grid-cols-1 xl:grid-cols-2 gap-10">
            <div class="space-y-4">
                <h3 class="text-[15px] font-black uppercase text-cyan-600 border-b border-cyan-100 pb-2">Kelengkapan Data Fenomena (Pengeluaran)</h3>
                <div class="h-[400px]">
            ...
            <div class="space-y-4">
    <h3 class="text-[15px] font-black uppercase text-cyan-600 border-b border-cyan-100 pb-2">Persentase Kelengkapan per Wilayah</h3>
    <div class="overflow-x-auto max-h-[400px] custom-scrollbar">
        <table class="w-full text-sm">
            <thead class="sticky top-0 bg-white">
                <tr class="border-b border-slate-200">
                ...
                @foreach($fenomenaLabelsPeng as $index => $label)
                    ...
                    <tr class="border-b border-slate-100 hover:bg-slate-50 transition">
                        ...
                        <td class="text-center py-3 px-2">
                            <div class="flex items-center justify-center gap-2">
                                <div class="w-16 bg-slate-100 rounded-full h-1.5">
                                    <div class="bg-cyan-500 h-1.5 rounded-full" style="width: {{ $persen }}%"></div>
                                </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-8 pt-6 border-t border-slate-100">
            ...
            <div class="bg-gradient-to-br from-cyan-50 to-white rounded-xl p-4 text-center">
                ...
                <p class="text-xs text-slate-500">{{ $jumlahWilayahPeng }} wilayah × {{ number_format($totalPerWilayahPeng) }} data</p>
            </div>
            ...
            <div class="bg-gradient-to-br from-slate-100 to-white rounded-xl p-4 text-center">
                <p class="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Kelengkapan Keseluruhan</p>
                <p class="text-2xl font-black {{ $targetTercapaiPeng ? 'text-emerald-600' : 'text-amber-600' }} mt-1">{{ $persenKeseluruhanPeng }}%</p>
                <p class="text-xs text-slate-500">{{ $targetTercapaiPeng ? 'Target tercapai (≥80%)' : 'Target 80%' }}</p>
            </div>
        </div>
    </div>
    @endif
