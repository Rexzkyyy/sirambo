...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($role != 'admin' && empty($kode_wilayah)) {
        $_SESSION['error'] = 'Kode wilayah wajib diisi untuk role ' . $role;
        header('Location: ../users.php');
        exit;
    }
    
    // Insert user baru dengan prepared statement (lebih aman)
    $stmt = $conn->prepare("INSERT INTO users (username, email, password, role, kode_wilayah, is_active, created_at, updated_at) 
                           VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
    
    if ($stmt) {
        $stmt->bind_param("sssssiis", $username, $email, $password, $role, $kode_wilayah, $is_active, $current_time, $current_time);
