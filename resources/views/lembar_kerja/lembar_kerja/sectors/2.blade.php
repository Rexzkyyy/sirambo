    ...
    <table class="w-full text-[11px] relative lk-sticky-table lk-dynamic-table" data-tipe-pdrb="{{ $tipePdrb }}">
        <thead class="bg-slate-100 text-slate-700 sticky top-0 z-10 shadow-[0_1px_0_0_rgba(0,0,0,0.1)]">
            <tr class="uppercase bg-slate-100 border-b border-slate-300">
                ...
                <th rowspan="2" class="border border-slate-300 px-2 py-1 min-w-[180px] text-left">Subkategori / Kegiatan / Komoditi</th>
                <th rowspan="2" class="border border-slate-300 px-1 py-1 w-16 text-center">Unit</th>
                <th colspan="3" class="border border-slate-300 px-1 py-1 text-center bg-blue-50 text-blue-900 font-bold text-[10px]">Output Utama</th>
                <th colspan="2" class="border border-slate-300 px-1 py-1 text-center bg-purple-50 text-purple-900 font-bold text-[10px]">Output Ikutan</th>
                <th colspan="3" class="border border-slate-300 px-1 py-1 text-center bg-amber-50 text-amber-900 font-bold text-[10px]">WIP (Work In Progress)</th>
                ...
                <th colspan="2" class="border border-slate-300 px-1 py-1 text-center bg-rose-50 text-rose-900 font-bold text-[10px]">Konsumsi Antara</th>
                ...
                <th rowspan="2" class="border border-slate-300 px-1 py-1 w-10 text-center bg-slate-100">Aksi</th>
            </tr>
            <tr class="uppercase bg-slate-50 text-[9px] text-slate-500 font-bold">
                <th class="border border-slate-300 px-1 py-1 w-20 text-center">Kuantum<br>(5)</th>
                <th class="border border-slate-300 px-1 py-1 w-24 text-center">Harga<br>(6)</th>
                <th class="border border-slate-300 px-1 py-1 w-24 text-center bg-blue-100/30">Nilai (7)<br>(5)x(6)</th>
                <th class="border border-slate-300 px-1 py-1 w-16 text-center">Rasio<br>(8)</th>
                <th class="border border-slate-300 px-1 py-1 w-24 text-center bg-purple-100/30">Nilai (9)<br>(8)x(7)</th>
                <th class="border border-slate-300 px-1 py-1 w-16 text-center">Deflator<br>(10)</th>
                <th class="border border-slate-300 px-1 py-1 w-24 text-center">WIP Berlaku<br>(11)</th>
                <th class="border border-slate-300 px-1 py-1 w-24 text-center bg-amber-100/30">Nilai WIP<br>(12)</th>
                <th class="border border-slate-300 px-1 py-1 w-16 text-center">Rasio<br>(14)</th>
        ...
        <tbody id="{{ $rowsId }}">
            @foreach($itemRows as $index => $item)
                <tr class="hover:bg-amber-50/50 transition-colors" data-template="tanaman_pangan" data-komoditas-id="{{ $item->komoditas_id }}">
                    ...
                    {{-- WIP --}}
                    <td class="border border-slate-200 px-1 py-1">
                        <input type="text" name="items[{{ $item->komoditas_id }}][deflator]" value="{{ \App\Helpers\PdrbHelper::formatId($item->deflator ?? 100, 4) }}" class="lk-input lk-dynamic-input w-full text-right" data-key="deflator">
                    </td>
                    <td class="border border-slate-200 px-1 py-1">
                        <input type="text" name="items[{{ $item->komoditas_id }}][wip_berlaku]" value="{{ \App\Helpers\PdrbHelper::formatId($item->wip_berlaku ?? 0, 2) }}" class="lk-input lk-dynamic-input w-full text-right" data-key="wip_berlaku">
                    </td>
                    <td class="border border-slate-200 px-1 py-1 bg-amber-50/40">
                        <input type="text" name="items[{{ $item->komoditas_id }}][wip]" value="{{ \App\Helpers\PdrbHelper::formatId($item->wip ?? 0, 2) }}" class="lk-input lk-dynamic-input w-full text-right font-bold text-amber-700" data-key="wip" data-formula="((wip_berlaku || 0) / (deflator || 100)) * 100" readonly>
                    </td>
        ...
        <tfoot class="sticky bottom-0 z-10">
            <tr class="bg-slate-800 text-white font-bold text-[10px] uppercase shadow-[0_-2px_4px_rgba(0,0,0,0.1)]">
                ...
                <td class="border border-slate-700 bg-blue-900/40 text-blue-300 px-2 py-3 text-right" {{ $totalPrefix }}="nilai_output_utama">0</td>
                <td class="border border-slate-700 bg-slate-700/50"></td>
                ...
                <td class="border border-slate-700 bg-slate-700/50"></td>
                <td class="border border-slate-700 bg-slate-700/50 text-right px-2" {{ $totalPrefix }}="wip_berlaku">0</td>
                <td class="border border-slate-700 bg-amber-900/40 text-amber-300 px-2 py-3 text-right" {{ $totalPrefix }}="wip">0</td>
                <td class="border border-slate-700 bg-sky-900/40 text-sky-300 px-2 py-3 text-right font-black" {{ $totalPrefix }}="output_adh">0</td>
                <td class="border border-slate-700 bg-slate-700/50"></td>
...
<template id="template-row-tanaman_pangan">
    <tr class="hover:bg-amber-50/50" data-template="tanaman_pangan">
        ...
        <td class="border border-slate-200 px-1 py-1 text-center text-slate-500 font-bold italic">UNIT</td>
        <td class="border border-slate-200 px-1 py-1">
        ...
        <td class="border border-slate-200 px-1 py-1 bg-amber-50/40">
            <input type="text" name="new_rows[INDEX][wip]" class="lk-input lk-dynamic-input w-full text-right font-bold text-amber-700 bg-transparent" data-key="wip" data-formula="((wip_berlaku || 0) / (deflator || 100)) * 100" readonly>
        </td>
        <td class="border border-slate-200 px-1 py-1 bg-sky-50/40">
