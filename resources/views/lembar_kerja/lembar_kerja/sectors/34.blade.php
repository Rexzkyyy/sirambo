<x-lk-sector-table :itemRows="$itemRows" :tipePdrb="$tipePdrb">
    ...
    @foreach($itemRows as $index => $item)
        <tr class="hover:bg-slate-50/40">
            ...
            <td class="border border-slate-200 px-1 py-1 bg-slate-50/50">
                <input type="text" name="items[{{ $item->komoditas_id }}][nilai_output_utama]" value="{{ \App\Helpers\PdrbHelper::formatId($item->nilai_output_utama, 4) }}" class="lk-input lk-dynamic-input w-full text-right font-bold" data-key="nilai_output_utama" data-formula="([kuantum]*[harga_produsen])">
            </td>
            <td class="border border-slate-200 px-1 py-1 bg-blue-50/50">
                <input type="text" name="items[{{ $item->komoditas_id }}][nilai_ntb]" value="{{ \App\Helpers\PdrbHelper::formatId($item->nilai_ntb, 4) }}" class="lk-input lk-dynamic-input w-full text-right font-bold text-blue-800" data-key="nilai_ntb" data-formula="[nilai_output_utama]">
            </td>
