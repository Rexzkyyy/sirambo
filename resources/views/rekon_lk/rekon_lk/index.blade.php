...
@section('content')
    ...
    <div class="p-1 h-full flex flex-col">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-2 mb-2">
            ...
            {{-- Filter Bar --}}
            <form method="get" class="filter-bar !mb-0 flex-shrink-0">
                <input type="hidden" name="jenis" value="{{ $jenis }}">

                @if($isProvinsiRole && $wilayahList->isNotEmpty())
                    <select name="wilayah_id" class="text-xs py-1.5 px-2">
                        @foreach($wilayahList as $w)
                            <option value="{{ $w->id_wilayah }}" {{ (int) $wilayahId === (int) $w->id_wilayah ? 'selected' : '' }}>
                                {{ $w->nama_wilayah }}
                            </option>
                        @endforeach
                    </select>
                ...
                <button type="submit" class="py-1.5 px-3">Terapkan</button>
                <a href="{{ url()->previous() }}" class="back-btn py-1.5 px-3">← Kembali</a>
            </form>
        </div>
                    ...
                    <tbody>
                        @php
                            ...
                            $fmtVal = function ($val, $isResultRow = false, $adjVal = 0, $isPercent = false) {
                                ...
                                if ($val < 0) {
                                    return '<span class="val-neg">(' . number_format(abs($val), 2, ',', '.') . ($isPercent ? '%' : '') . ')</span>';
                                }
                                return number_format($val, 2, ',', '.') . ($isPercent ? '%' : '');
                            };
                        @endphp
                        ...
                        @foreach($items as $item)
                            @if(count($items) > 1)
                                <tr class="bg-slate-100 border-t-2 border-slate-300">
                                    <td colspan="{{ 2 + (count($tablesData) * (count($periodeCols) + 1)) }}" class="px-3 py-2 text-sm font-bold text-slate-800 bg-slate-200 uppercase sticky left-0 z-20">
                                        {{ $item['kode'] }} {{ $item['nama'] }}
                                    </td>
                                </tr>
                            @endif
        ...
        <script>
            $(document).ready(function () {
                function formatNumber(val, isResultRow = false, adjVal = 0, isPercent = false) {
                    ...
                    let neg = num < 0;
                    let str = Math.abs(num).toLocaleString('id-ID', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
                    if (isPercent) str += '%';
