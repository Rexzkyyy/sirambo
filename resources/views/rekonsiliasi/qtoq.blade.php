...
@section('title', 'Pertumbuhan Q to Q')
...
<div class="p-4 md:p-8 space-y-6 w-full max-w-none mx-auto">
        ...
        <div class="flex flex-col md:flex-row justify-between items-center gap-4">
            <div class="space-y-1 text-center md:text-left">
                ...
                <h1 class="text-xl md:text-2xl font-black text-slate-900 leading-tight">
                    Pertumbuhan <span class="text-blue-600">Q to Q</span> {{ $isLapanganUsaha ? 'Lapangan Usaha' : 'Pengeluaran' }}
                </h1>
                
                <p class="text-slate-500 text-xs md:text-sm font-medium">
                    Pertumbuhan Q-to-Q PDRB Kabupaten/Kota Provinsi berdasarkan
                    <span class="text-slate-900 font-semibold">{{ $isLapanganUsaha ? 'Lapangan Usaha' : 'Pengeluaran' }}</span>
                </p>
            </div>

            <div class="flex items-center gap-3">
                <a href="{{ route('rekonsiliasi.export','qtoq') }}?{{ http_build_query(request()->all()) }}"
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4 w-full">
                ...
                <div class="space-y-1.5">
                    ...
                    <select name="pendekatan" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm font-semibold text-slate-700 focus:ring-2 focus:ring-blue-500/20 outline-none transition-all">
                        <option value="lapangan_usaha" {{ $pendekatan === 'lapangan_usaha' ? 'selected' : '' }}>🏗️ Lapangan Usaha</option>
    ...
    {{-- ================= INSIGHT SECTION ================= --}}
    @php
        $luRowInsight = $grouped->firstWhere('kategori', 'PRODUK DOMESTIK REGIONAL BRUTO');
        $lapusRowInsight = $grouped->firstWhere('kategori', 'PRODUK DOMESTIK REGIONAL BRUTO LAPUS');
        ...
        <div class="custom-table-container overflow-auto max-h-[650px] relative">
            <table id="main-table" class="w-full border-separate border-spacing-0 text-sm">
                ...
                <tbody class="divide-y divide-slate-200">
                    @foreach($grouped->where('hidden', '!=', true) as $row)
                        @php
                        ...
                        <tr class="hover:bg-blue-50/50 transition-colors {{ $row['is_sub'] ? 'bg-white' : 'bg-slate-50/80 font-bold' }}">
                            <td class="sticky left-0 z-20 px-6 py-3 border-r border-b border-slate-200 {{ $row['is_sub'] ? 'bg-white' : 'bg-slate-50' }}">
                                <div class="flex items-start gap-3">
                                    {{-- Kode diletakkan di sisi kiri dengan gaya sticky side --}}
                                    <span class="text-[10px] font-black text-slate-400 w-12 shrink-0 mt-1">
                                        {{ $row['kode'] ?? '' }}
                                    </span>
                ...
                @php
                    $luRow = $grouped->firstWhere('kategori', 'PRODUK DOMESTIK REGIONAL BRUTO');
                    $lapusRow = $grouped->firstWhere('kategori', 'PRODUK DOMESTIK REGIONAL BRUTO LAPUS');
                @endphp
                ...
                <tfoot class="bg-slate-100 font-black">
                    <tr class="border-t-2 border-slate-300">
                        ...
                        @php
                            $provSel = round($luRow['provinsi'] - $lapusRow['provinsi'], 9);
                            $totalSel = round($luRow['total_kab'] - $lapusRow['total_kab'], 9);
                        @endphp
